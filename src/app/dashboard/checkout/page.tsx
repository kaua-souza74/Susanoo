...
import { CreditCard, QrCode, CheckCircle2, ShieldCheck, ArrowLeft, Lock, ShoppingBag } from "lucide-react";
    ...
    return (
      <div className="flex-1 w-full h-full bg-background flex flex-col items-center justify-center p-6 text-center">
        <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="flex flex-col items-center">
          <div className="w-24 h-24 bg-emerald-500/10 border-2 border-emerald-500 flex items-center justify-center rounded-full mb-6">
            <CheckCircle2 className="w-12 h-12 text-emerald-500" />
          </div>
          <h2 className="text-3xl font-black mb-3">Pagamento Aprovado!</h2>
          <p className="text-foreground/60 max-w-md mx-auto mb-8">
            Seus projetos já estão disponíveis no seu painel. Redirecionando para os seus projetos...
          </p>
      ...
      <div className="max-w-6xl w-full mx-auto py-8 px-4 grid grid-cols-1 lg:grid-cols-12 gap-10">
        ...
        <div className="lg:col-span-7 flex flex-col gap-8">
          ...
          <AnimatePresence mode="wait">
              ...
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="flex flex-col gap-8">
                ...
                <div className="grid grid-cols-2 gap-5">
                  <div className="col-span-2">
                    <label className="block text-xs font-bold uppercase tracking-wider opacity-60 mb-2">Número do Cartão</label>
                    <input 
                      type="text" 
                      value={cardNumber}
                      ...
                      className="w-full bg-surface border border-surface-border rounded-xl p-4 font-medium focus:border-accent outline-none"
                    />
                  </div>
                  <div className="col-span-2">
                    <label className="block text-xs font-bold uppercase tracking-wider opacity-60 mb-2">Nome do Titular</label>
                    <input 
                      type="text" 
                      value={cardName}
                      ...
                      className="w-full bg-surface border border-surface-border rounded-xl p-4 font-medium focus:border-accent outline-none uppercase"
                    />
                  </div>
                  <div>
                    <label className="block text-xs font-bold uppercase tracking-wider opacity-60 mb-2">Validade</label>
                    <input 
                      type="text" 
                      value={cardExpiry}
                  ...
                  <div>
                    <label className="block text-xs font-bold uppercase tracking-wider opacity-60 mb-2">CVC</label>
                    <input 
                      type="text" 
                      value={cardCVC}
                      maxLength={4}
            ...
            {method === "pix" && (
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="bg-surface border border-surface-border rounded-3xl p-8 flex flex-col items-center text-center">
                ...
                <h3 className="font-black text-lg mb-2">Escaneie o QR Code</h3>
                ...
                <div className="w-full bg-background border border-surface-border p-4 rounded-xl flex items-center justify-between gap-4">
                  <span className="font-mono text-xs opacity-50 truncate">00020101021226880014br.gov.bcb.pix...</span>
                  <button className="text-xs font-black uppercase tracking-wider text-emerald-500 hover:text-emerald-400 bg-emerald-500/10 px-4 py-2 rounded-lg shrink-0 cursor-pointer">Copiar Copia e Cola</button>
                </div>
              </motion.div>
            )}
          </AnimatePresence>
        ...
        <div className="lg:col-span-5">
          <div className="bg-surface border border-surface-border rounded-3xl p-6 lg:p-8 sticky top-24">
                ...
                <div key={item.id} className="flex items-center gap-4">
                  <div className="w-16 h-16 bg-background rounded-xl border border-surface-border overflow-hidden shrink-0">
                    {item.cover_url ? (
                      ...
                      <div className="w-full h-full flex items-center justify-center font-black opacity-20">{item.name.substring(0,3)}</div>
                    )}
                  </div>
                  ...
                  <div className="text-right">
                    <p className="font-black">R$ {item.price.toFixed(2).replace('.',',')}</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            <button 
              ...
              className="w-full mt-8 py-5 bg-accent text-white font-black rounded-xl uppercase tracking-widest shadow-xl shadow-accent/20 hover:bg-accent/90 hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
            >
